...
function App(){const[logged,setLogged]=useState(()=>sessionStorage.getItem(SESSION)==="1"),[data,setData]=useState(load),[tab,setTab]=useState("dashboard"),[clock,setClock]=useState(new Date()),[search,setSearch]=useState(""),[alertSearch,setAlertSearch]=useState(""),[saved,setSaved]=useState("Sin cambios"),[selectedInvoiceId,setSelectedInvoiceId]=useState(null),[selectedMonth,setSelectedMonth]=useState(mk(today())),[invoiceFolderMonth,setInvoiceFolderMonth]=useState(mk(today())),[invoiceStatusFilter,setInvoiceStatusFilter]=useState("Todas"),[invoicePage,setInvoicePage]=useState(1),[chatOpen,setChatOpen]=useState(true),[chatInput,setChatInput]=useState(""),[emailSending,setEmailSending]=useState(false),[chatMessages,setChatMessages]=useState([{role:"ia",text:"Hola, soy la IA de Cobranza GPSRUTA. Pregúntame: ¿quién debe más?, ¿facturas vencidas?, ¿clientes premium?, ¿resumen del mes?, ¿ingresos?, ¿egresos?"}]);
...
const alertInvoices=data.invoices.filter(i=>isVencida15(i)||isPorVencer15(i)).filter(i=>`${i.factura} ${client(i.clienteId)?.nombre||""}`.toLowerCase().includes(alertSearch.toLowerCase()));
...
function askDashboardAI(){
  ...
  if(lower.includes("pendiente")||lower.includes("pendientes")){ const lista=aiData.riesgoCliente.filter(c=>c.pendientes>=2).slice(0,10); answer=lista.length?"Clientes con 2 o más facturas pendientes:\n"+lista.map(c=>`• ${c.nombre}: ${c.pendientes} pendiente(s), riesgo ${c.riesgo}`).join("\n"):"No hay clientes con 2 o más facturas pendientes."; }
...
function Table({rows,type,setData,data}){let sorted=[...rows].sort((a,b)=>(b.fecha||b.vencimiento).localeCompare(a.fecha||a.vencimiento));return <div className="tableWrap"><table><thead><tr><th>Fecha</th><th>Mes/Año</th><th>Categoría</th><th>Descripción</th><th>Monto</th><th>Acciones</th></tr></thead><tbody>{sorted.map(r=><tr key={r.id}><td>{r.fecha||r.vencimiento}</td><td>{ml(mk(r.fecha||r.vencimiento))}</td><td>{r.categoria}{r.proveedor&&<small>{r.proveedor}</small>}{r.emailProveedor&&<small>{r.emailProveedor}</small>}</td><td>{r.descripcion}{r.estado&&<small>Estado: {r.estado}</small>}</td><td>{money(r.monto)}</td><td>{type==="debts"&&<a className="icon mail" href={debtEmail(r)} title="Correo manual deuda"><Mail size={17}/></a>}{type==="debts"&&<button className="icon autoMailIcon" onClick={()=>sendAutoDebt(r)} title="Enviar deuda automático">AUTO</button>}<button className="icon trash" onClick={()=>setData({...data,[type]:(data[type]||[]).filter(x=>x.id!==r.id)})}><Trash2 size={17}/></button></td></tr>)}</tbody></table></div>}
